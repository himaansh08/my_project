...
<div class="d-flex align-items-center gap-2">
    ...
    <div>
        ...
        <button type="button" id="clear-search" class="btn btn-outline-secondary">Clear</button>
    </div>
</div>
...
<div class="table-container">
    ...
    <div class="search-table" style="display: none;"></div>
</div>
...
function search() {
    ...
    if (searchText === '') {
        // If search is empty, show original table and hide search results
        $('.original-table').show();
        $('.search-table').hide();
        $('.pagination').show();
        return;
    ...
    // Perform AJAX search for all records
    $.ajax({
        ...
        success: function(response) {
            if (response.includes('No results found')) {
                showErrorMessage('No matching users found.');
                $('.search-table').hide();
                $('.original-table').show();
            } else if (response.includes('Please enter a search term')) {
                showErrorMessage('Please enter a search term.');
            } else if (response.includes('Error executing search query')) {
                showErrorMessage('Error executing search. Please try again.');
            } else {
                // Show search results table and hide original table
                $('.search-table').html(response).show();
                $('.original-table').hide();
                $('.pagination').hide(); // Hide pagination during search
                showSuccessMessage('Search completed successfully.');
            }
        }
    });
}
...
$(document).ready(function() {
    ...
    if ($('#clear-search').length === 0) {
        $('#search-bar').after('<button type="button" id="clear-search" class="btn btn-outline-secondary ms-2">Clear</button>');
    }
    ...
    // Rest of your existing delete functionality...
    $(document).on('click', '#delete_customer', function(event) {
        ...
        if (confirm('Sure you want to delete the customer?')) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        showSuccessMessage(response.message + ' Redirecting...');
                        setTimeout(function() {
                            location.reload();
                        }, 2000);
                    } else {
