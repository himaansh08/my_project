...
<script>
function search() {
    var searchText = $('#search-bar').val().trim().toLowerCase();
    var visibleRows = 0;
    
    // Clear previous messages
    $('#responseMessage').html('');
    
    // Loop through each table row (except header)
    $('.table tbody tr').each(function() {
        var row = $(this);
        var firstName = row.find('td:nth-child(2)').text().toLowerCase();
        var lastName = row.find('td:nth-child(3)').text().toLowerCase();
        var email = row.find('td:nth-child(4)').text().toLowerCase();
    ...
    if (searchText !== '' && visibleRows === 0) {
        showErrorMessage('No matching users found.');
    } else if (searchText !== '' && visibleRows > 0) {
        showSuccessMessage(`Found ${visibleRows} matching user(s).`);
    }
}
...
$(document).ready(function() {
    ...
    // Rest of your existing code for delete functionality remains the same...
