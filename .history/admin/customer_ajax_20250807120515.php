...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] == 'search_users') {
        ...
        if ($search !== ' ') {
            ...
            $stmt = $conn->prepare("SELECT * FROM users_info WHERE first_name LIKE ? OR email LIKE ?");
            $stmt->bind_param("ss", $search, $search);
            $stmt->execute();
            $result = $stmt->get_result();

            if ($result->num_rows > 0) {
                echo "<table class='table table-bordered'><thead><tr><th>First Name</th><th>Last Name</th><th>Email</th></tr></thead><tbody>";
